...
class GraphSage(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, nlayers=2, dropout=0.5, lr=0.01, weight_decay=5e-4,
            with_relu=True, with_bias=True, with_bn=False, device=None, **kwargs):
    ...
    def fit_with_val(self, features, adj, labels, data, train_iters=200, initialize=True, verbose=False, normalize=True, patience=None, noval=False, **kwargs):
        ...
        if len(labels.shape) > 1:
            ...
            def custom_loss(logits, targets):
                ...
                #print(F.log_softmax(logits, dim=-1).detach().cpu().numpy().max(), F.log_softmax(logits, dim=-1).detach().cpu().numpy().min())
                #print(-torch.mean(torch.sum(F.log_softmax(logits, dim=-1) * targets, dim=-1)))
    ...
    def _train_with_val(self, labels, data, train_iters, verbose, adj_val=False):
        ...
        #pdb.set_trace()
        # if adj_norm.density() > 0.5: # 0.5 if the weighted graph is too dense, we need a larger neighborhood size
        #    sizes = [30, 20]
        ...
        for i in range(train_iters):
            ...
            for batch_size, n_id, adjs in train_loader:
                ...
                loss_train = self.loss(out, labels[n_id[:batch_size]]) # F.nll_loss(out, labels[n_id[:batch_size]])
                loss_train.backward()
                optimizer.step()
